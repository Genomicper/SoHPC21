...
profvis({
    ...
    all_data <- read_pvals_v2(data_name = dataname, 
                           snps_ann = ann,
                           from = "workspace")
    
    # Order data according to the genome
    genome_results <- genome_order_vs2(all_data = all_data)
    ...
    #qq <- plot_results_v2(results = results, by = "set", plot_all = TRUE)
...
gwas <- vroom(file = "Data/GWAS_30K.txt")
tgwas <- fread(file = "Data/GWAS_30K.txt")
object_size(gwas)   # On Irem's laptop 2,208 B
object_size(tgwas)  # On Irem's laptop 2,399,304 B
...
res <- microbenchmark(
  read_pvals(data_name = dataname, snps_ann = ann, from = "workspace"),
   read_pvals_v2(data_name = dataname, snps_ann = ann,from = "workspace", verbose = TRUE),
  read_pvals_v2(data_name = dataname, snps_ann = ann,from = "workspace"),
  unit = "s",
  times = 20
)
