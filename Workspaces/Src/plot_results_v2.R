...
function(results = "", by="", plot_all = TRUE, var = "", save_plot = TRUE,
    ...
    if (plot_name == "") {
       cat("Save plot as (default):Results_genomicper_", substitute(by),
           sep = "")
    }
    ...
    if (plot_all == TRUE & save_plot == FALSE) {
      stop("If plot_all is set to TRUE, the plots must be saved in working
           directory (\"save_plot=TRUE\")")
...
plot(expL, empL, xlim = c(0, mxx), ylim = c(0, mxy), main = traits[i],
     xlab = "Expected(-log10)", ylab = "Observed(-log10)", col = "red")
...
plot(expL, empL, xlim = c(0, mxx), ylim = c(0, mxy), main = var,
     xlab = "Expected(-log10)", ylab = "Observed(-log10)", col = "red")
lines(expL, expL, type = "l")
if (bf == TRUE) {
abline(h = -log10(bf1), col = "blue", cex = 1.2)
...
plot(expL, empL, xlim = c(0, mxx), ylim = c(0, mxy), main = paths[i],
     xlab = "Expected(-log10)", ylab  = "Observed(-log10)", col = "red")
lines(expL, expL, type = "l")
if (bf == TRUE) {
abline(h = -log10(bf1), col = "blue", cex = 1.2)
