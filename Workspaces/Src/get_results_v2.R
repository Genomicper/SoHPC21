...
function(res_pattern = "Permus", level = "snp", from = "workspace",
        ...
        for (i in list.files(pattern = res_pattern)) {
            ...
            if (x %in% permus_list) {
                ...
                assign(permus_list[z], cbind(get(permus_list[z], envir = envir),
                       fread(i, show_col_types = FALSE,data.table = FALSE)), envir = envir)
            } else {
                assign(mode, fread(i, show_col_types = FALSE,data.table = FALSE), envir = envir)
    ...
    if (level == "gene") {
        ...
        for (i in 1:dim(get(permus_list[1], envir = envir))[2]) {
            reslts <- c(reslts,
                        strsplit(colnames(get(permus_list[1],
                                                      envir = envir)),
                                 "_")[[i]][3])
        }
        trts <- NULL
        for (i in seq_len(length(permus_list))) {
